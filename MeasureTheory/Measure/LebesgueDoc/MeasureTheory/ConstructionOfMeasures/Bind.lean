...
$$`
  \Big\{ \{ \mu \in \Measure(X) \mid \mu(A) \in B \} \Bigm|
    \text{$A \subseteq X$ and $B \subseteq [0,\infty]$ are measurable} \Big\}.`
...
$$`
  \Big\{ \{ \mu \in \Measure(X) \mid \mu(A) \in B \} \Bigm|
    \text{$A \subseteq X$ と $B \subseteq [0,\infty]$ は可測} \Big\}
`
が生成する σ-代数を入れることで、$`\Measure(X)` を可測空間とみなす。
